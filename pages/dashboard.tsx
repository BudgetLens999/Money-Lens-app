  ...
  useEffect(() => {
    ...
    supabase.auth.getUser().then(async ({ data: { user } }) => {
      ...
      try {
        ...
        setAccess('active') // fail open during development
      }
    })
  }, [router])
        ...
        <div className="min-h-screen bg-stone-50 flex flex-col items-center justify-center px-4">
          ...
          <div className="card p-8 max-w-md w-full text-center">
            ...
            <p className="text-stone-500 text-sm mb-6">
              Subscribe to continue tracking your finances.
            </p>
            ...
            <button onClick={handleLogout} className="text-stone-400 text-xs hover:text-stone-600 mt-4 block mx-auto">
              Sign out
            </button>
          </div>
        </div>
      ...
      <div className="min-h-screen bg-stone-50">
        ...
        <div className="max-w-5xl mx-auto px-6 py-10">
          <div className="card p-8 text-center">
            <div className="text-4xl mb-4">📊</div>
            <h2 className="text-xl font-semibold text-stone-900 mb-2">You're in!</h2>
            <p className="text-stone-500 text-sm mb-6">
              Your MoneyLens account is active. The full budget tracker is being integrated here.
            </p>
            <p className="text-stone-400 text-xs">Signed in as {user?.email}</p>
          </div>
        </div>
      </div>
    </>
